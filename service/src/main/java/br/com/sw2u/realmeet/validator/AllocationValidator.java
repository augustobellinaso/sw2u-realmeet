...
import static br.com.sw2u.realmeet.util.DateUtils.isOverlapping;
...
public class AllocationValidator {
    ...
    private void validateSubject(String subject, ValidationErrors validationErrors) {
        ...
        validateMaxLength(subject, ALLOCATION_SUBJECT, ALLOCATION_SUJECT_MAX_LENGTH, validationErrors);
    }
    ...
    private void validateDates(Long roomId, OffsetDateTime startAt, OffsetDateTime endAt, ValidationErrors validationErrors) {
        if (validateDateIsPresent(startAt, endAt, validationErrors)) {
            ...
            validateIfTimeIsAvailable(roomId, startAt, endAt, validationErrors);
        }
    }
    
    private boolean validateDateIsPresent(OffsetDateTime startAt, OffsetDateTime endAt, ValidationErrors validationErrors) {
        return (validateRequired(startAt, ALLOCATION_START_AT, validationErrors) && validateRequired(endAt, ALLOCATION_END_AT, validationErrors));
    }
    ...
    private void validateDuration(OffsetDateTime startAt, OffsetDateTime endAt, ValidationErrors validationErrors) {
        if (between(startAt, endAt).getSeconds() > ALLOCATION_MAX_DURATION_SECONDS) {
